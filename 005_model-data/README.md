...
The amount of light that is reflected off of the surface of an object is related to the orientation of the surface relative to the source. If the surface gets hit straight on, most of the light will be reflected to all possible directions. If the light just touches the side of an object, then very little light is reflected. For the *diffuse light*, the amount of reflection is dependent on an angle between the surface normal vector and the light ray, calculated as cosine of that angle. When the angle is 0° all the light is reflected, when the angle is 90° then no light is reflected. If you multiply the color of surface by the cosine of this angle, you will scale the color towards black, meaning the less light reflects off of a surface, the darker the surface becomes.

If an object is smooth, some of the light reflected off of the surface can be reflected directly into the view point. This creates *specular highlight* that is the color of the light source, not the color of the object, because you are actually seeing the light source light. The location of a specular highlight is determined by the angle between a ray from the viewer to the point on the surface and the exact reflection of the light ray. 

In these previous explanations we calculated the colors through the surface normal, where every pixel on a face has the same color, referred as *flat shading*. There are however also other methods to achieve even more realistic results. In *Gouraud shading*, a different normal vector can be specified for each vertex, that way colors inbetween will be interpolated from the vertex colors. If the vertex normal vectors are set correctly, the face will have a curved appearance. You can get even more accurate coloring across a face if you interpolate the normal vectors from the vertices accross the face and then recalculate the fragment coloro using the interpolated vector. This method, called *Phong shading*, gives smoother color gradients but takes longer to compute. Both Gouraud and Phong techniques are called *smooth shading*.

Other intuitive method is controlling the color of individual pixels by specifying a unique normal vector for each pixel. This is done by creating an image, where each point in the image is treated as an `<dx, dy, dz>` normal vector rather than a RGB value. This is called a *bump mapping*, simulating small displacements of the surface while the geometry is not modified. You can distinguish between a smooth object with bump mapping and an object with actual geometry modification by looking at its shadow - the object with bump map cast a smooth shadow without surface displacements.
